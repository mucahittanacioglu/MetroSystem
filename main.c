...
int main()
{
    ...
    // testing neighboring station func.
    MetroStation ssa[SIZE]={'\0'};
    getNeighboringStations(istanbul,s2,ssa);
    printf("The neighbor stations of %s are: \n",s2.name);
    printPath(ssa);

    printf("\nThe best path from %s to %s is:\n", nearMe.name, nearGoal.name);
    ...
    findPath(nearMe, nearGoal, myPath,istanbul);

    if(strlen(myPath[SIZE-1].name) == 0)
